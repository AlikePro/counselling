...
left_panel_open = st.session_state.get("left_panel_open", True)
info_panel_open = st.session_state.get("info_panel_open", True)
left_padding = "260px" if left_panel_open else "24px"
right_padding = "380px" if info_panel_open else "24px"
panel_class = "panel-open" if info_panel_open else "panel-closed"
sidebar_transform = "translateX(0)" if left_panel_open else "translateX(-100%)"
...
.block-container {{
    ...
    padding-left: {left_padding} !important;
    padding-right: {right_padding} !important;
}}
...
if st.button("Toggle overview panel"):
...
with controls[0]:
    if not st.session_state.get("left_panel_open", True):
        if st.button("☰ Menu"):
            st.session_state.left_panel_open = True
            st.rerun()
with controls[1]:
...
with st.sidebar:
    ...
    if st.button("☰ Collapse menu"):
        ...
        st.rerun()
    active_tab = st.radio("Navigation", tab_labels, label_visibility="collapsed", key="active_tab")
